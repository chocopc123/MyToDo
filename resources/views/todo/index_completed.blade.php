...
@section('content')
  ...
  <div class="col-12 col-sm-12 col-md-3 col-xl-2 order-md-first" style="background-color: #e3f2fd;">
    <ul class="list-group">
      ...
      <a href="/index/refine/all" class="list-group-item list-group-item-action font-weight-bold  <?php if(session('refine')=='/'){ echo "active"; } ?>">達成済み一覧 <span class="sr-only">(current)</span></a>
    ...
    <ul class="list-group">
      ...
      <a href="/index/sort/completed_date" class="list-group-item list-group-item-action font-weight-bold <?php if(session('sort')=='completed_date'){ echo "active"; } ?>">達成日時 <?php if(session('sort')=='completed_date'){ echo '[' . session('order') . ']'; } ?></a>
    </ul>
  </div>
@endsection
